...
class IranNetworkDetector:
    ...
    def assess_mining_threat(self, device_info):
        ...
        try:
            ...
            'geographic_distribution': {},
            'port_analysis': {},
            'confidence_stats': {'avg': 0, 'max': 0, 'min': 100}
        }
        
        confidences = []
        
        for device in detected_devices:
            mining_indicators = device.get('mining_indicators', {})
            confidence = mining_indicators.get('confidence_score', 0)
            threat_level = mining_indicators.get('threat_level', 'low')
            
            confidences.append(confidence)
            
            if device.get('threat_assessment', {}).get('is_miner'):
                analysis['miners_found'] += 1
            elif confidence >= 30:
                analysis['suspicious_devices'] += 1
            
            # Threat level breakdown
            analysis['threat_breakdown'][threat_level] += 1
            ...
            for port in device.get('open_ports', []):
                if port in self.mining_ports:
                    analysis['port_analysis'][port] = analysis['port_analysis'].get(port, 0) + 1
    ...
    def store_scan_results(self, detected_devices, config):
        ...
        try:
            ...
            for device in detected_devices:
                threat_assessment = device.get('threat_assessment', {})
                mining_indicators = device.get('mining_indicators', {})
                geo = device.get('geolocation', {})
                
                if threat_assessment.get('is_miner') or mining_indicators.get('confidence_score', 0) >= 50:
                    # Store as detected miner
                    cursor.execute("""
                        INSERT OR REPLACE INTO miners 
                        (ip, mac, device_type, open_ports, suspicion_score, city, country, 
                         latitude, longitude, hostname, status, detection_time, 
                         power_consumption, hash_rate, response_time, is_active, notes)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        device['ip_address'],
                        device.get('mac_address'),
                        mining_indicators.get('mining_type', 'unknown'),
                        ','.join(map(str, device.get('open_ports', []))),
                        int(mining_indicators.get('confidence_score', 0)),
                        geo.get('city'),
                        geo.get('country'),
                        geo.get('lat'),
                        geo.get('lon'),
                        ...
                        None,  # power_consumption - would need hardware sensors
                        None,  # hash_rate - would need API access
                        None,  # response_time
                        1,
                        json.dumps({
            ...
            print(f"Error storing results: {e}", file=sys.stderr)
        finally:
            conn.close()
