            ...
            {filteredMiners.length === 0 ? (
              ...
              filteredMiners.map((miner) => (
                ...
                >
                  ...
                  {/* اگر miner.geoip یا miner.isp وجود دارد، نمایش بده */}
                  {miner.geoip && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      <span className="font-medium">GeoIP:</span> {miner.geoip}
                    </div>
                  )}

                  {/* اگر miner.latitude و miner.longitude وجود دارد، لینک به نقشه اضافه کن */}
                  {miner.latitude && miner.longitude && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      <span className="font-medium">موقعیت مکانی:</span>
                      <a
                        href={`https://maps.google.com/?q=${miner.latitude},${miner.longitude}`}
                        target="_blank"
